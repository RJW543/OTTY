#!/usr/bin/env python3
"""
OTP Kiosk Launcher - Standalone Version with Voice Support
...
class KioskLauncher:
    ...
    def setup_ui(self):
        ...
        subtitle = tk.Label(
            main,
            text="Encrypted Messaging & Voice Calls",
            font=("Helvetica", 12),
            fg='#8b949e',
            bg='#0d1117'
        )
        ...
        # Button container - two columns for direct launch
        ...
        # Style for buttons
        def create_button(parent, text, command, color='#238636', width=20):
            btn = tk.Button(
                parent,
                text=text,
                command=command,
    ...
    def check_dependencies(self):
        ...
        if missing:
            self.deps_status.config(
                text=f"⚠️ Optional: pip install {' '.join(missing)}",
                fg='#d29922'
            )
        else:
            self.deps_status.config(
...
def main():
    ...
    optional = ["otp_relay_server.py", "otp_relay_server_voice.py", "otp_voice_client.py", "otp_generator.py"]
    ...
    if missing_optional:
        messagebox.showwarning(
            ...
            f"The following optional files were not found in {APP_DIR}:\n\n" +
