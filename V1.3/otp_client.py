...
class OTPClientGUI:
    ...
    def setup_ui(self):
        ...
        if recipient_from_env:
            self.recipient_entry.insert(0, recipient_from_env)
            if recipient_name and recipient_name != recipient_from_env:
                # Show contact name
                self.recipient_name_label = ttk.Label(
                    recipient_row, 
                    text=f"({recipient_name})",
                    foreground='#0066cc'
                )
                self.recipient_name_label.pack(side=tk.LEFT, padx=(10, 0))
